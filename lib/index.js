...
exports.download = (opts, url, reporter = () => {}) => {
  ...
  return new Promise((resolve, reject) => {
    request.head(url, (err, res, body) => {
      ...
      if (
        res.request.uri.pathname.indexOf('photo-1446704477871-62a4972035cd') >=
        0
      ) {
        reject({
      ...
      const pathname = res.request.uri.pathname
        .slice(1)
        .replace(/[\/\\]/g, '-');
      const uniqueName = path.join(dir, `wallpaper-${pathname}.jpg`);
