...
const { options, commands, aliases } = require('./args');
...
exports.sanitizeArgs = (args, reporter) => {
  const flags = args._.reduce((acc, arg) => {
    if (commands.indexOf(arg) > -1) {
      acc[arg] = true;
    } else {
      reporter(`"${arg}" is not a valid command`);
    }
    return acc;
  }, {});
  const opts = Reflect.ownKeys(args).reduce((acc, key) => {
    const option = options[key];
    if (option) {
      if (!option.test || option.test(args[key], args)) {
        acc[key] = option.transform ? option.transform(args[key]) : args[key];
      } else {
  ...
  return Object.assign({}, flags, opts);
};
...
exports.createUrl = opts => {
  ...
  return `https://source.unsplash.com${pieces}${more}`;
};
